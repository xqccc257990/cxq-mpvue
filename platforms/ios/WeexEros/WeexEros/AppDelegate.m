...
#include "extend/NSBundle+language.h"
#import "BMDB.h"
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    if ([[NSUserDefaults standardUserDefaults] objectForKey:@"myLanguage"] && ![[[NSUserDefaults standardUserDefaults] objectForKey:@"myLanguage"] isEqualToString:@""]) {
        ...
        [BMGetEnvironment setLocale: [language componentsSeparatedByString:@"-"][0]];
    } else {
        NSArray *languages = [[NSUserDefaults standardUserDefaults] objectForKey:@"AppleLanguages"];
        NSArray *language = [languages[0] componentsSeparatedByString:@"-"];
        if ([language[0] isEqualToString:@"zh"]) {
            [BMGetEnvironment setLocale: @"zh"];
        } else if ([language[0] isEqualToString:@"en"]) {
            [BMGetEnvironment setLocale: @"en"];
        }
    }
    ...
    [WXSDKEngine registerModule:@"zyTools" withClass: NSClassFromString(@"WXZyToolsModule")];
    [WXSDKEngine registerComponent:@"scroller-header" withClass: NSClassFromString(@"WXScrollerHeaderComponent")];
